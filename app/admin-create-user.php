...
<body class="bg-dark text-white">
    ...
    <script>
        ...
        function displayToastr(status, message) {
            ...
            try {
                ...
                if (status === 'success') {
                 ...
                 // Fallback caso a decodificação falhe (se não for base64)
                 let decodedMessage = decodeURIComponent(message.replace(/\+/g, ' '));
                 toastr.error(decodedMessage, 'Erro no Processamento');
            }
        }
        ...
        if (status && message) {
            ...
            // Limpa os parâmetros da URL após exibição para evitar re-trigger
